...
const JustificationForm = ({ masterId }: { masterId: string }) => {
  ...
  const apiUrl = import.meta.env.VITE_API_URL;
    ...
    const fetchData = async () => {
        try {
        const justificationRes = await fetch(`${apiUrl}/justifications/master/${masterId}`);
        const justificationData = await justificationRes.json();
        const justifiedAttendanceIds = new Set(
            justificationData.justifications.map((j: any) => j.attendance_id)
        );
  ...
  const handleSubmit = async (attendance_id: string, status: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setAttendances((prev) => prev.filter((att) => att.id !== attendance_id));
      ...
      {attendances.map((att, index) => (
        ...
        >
          <h3 className="font-bold text-2xl mb-3 text-white drop-shadow-md">
            Estado: {att.status === "R" ? "Retardo" : "Inasistencia"}
          </h3>
          ...
          <textarea
            ...
            onChange={(e) => handleJustificationChange(att.id, e.target.value)}
          />
