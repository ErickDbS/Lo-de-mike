...
interface CheckCardProps {
  ...
  Tema: string;
  Hora: string;
  status: 'A' | 'NA' | 'R' | string;
}

export default function CheckCard({ nameMateria, nameProfesor, Hora, Tema, status }: CheckCardProps) {
  const asistio = status === 'A';
        ...
        <div className="flex justify-center gap-4">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xl transition flex items-center gap-2 ${
              asistio ? 'text-green-600' : 'text-red-600'
            }`}
            disabled
          >
            {asistio ? <Check size={20} /> : <X size={20} />} {asistio ? 'Asistió' : 'No asistió'}
          </button>
        </div>
      </div>
    </div>
