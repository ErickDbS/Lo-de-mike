...
const userRole: any = authContext.storage.role;
  ...
  useEffect(() => {
    ...
    setFilteredJustifications(
      justifications.filter(
        j => attendanceIds.includes(j.attendance_id) && j.status === 'P'
      )
    );
  ...
  const handleReview = async (justId: number, status: 'A' | 'R') => {
    ...
    const reviewerId = localStorage.getItem('enrollment_number') || 'sin id';
    ...
    try {
      const res = await fetch(`${API_BASE}/justifications/review/${justId}`, {
        method: 'PATCH',
      ...
      setFilteredJustifications(prev => prev.filter(j => j.id !== justId));

      Swal.fire('Éxito', `Justificación ${verb} correctamente.`, 'success');
    } catch (err) {
